...
/** Re-mail the admin login code for a still-open challenge. */
export async function POST(req: Request) {
  const limited = await rateLimitGuard(req, "admin-otp-resend", 6, 10 * 60_000);
  ...
  if (!otp || otp.consumedAt || !STAFF_ROLES.includes(otp.user.role)) {
